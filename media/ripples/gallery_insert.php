...
<?
	...
	if ($mode=="modify")
	{
		...
		$instar = htmlspecialchars($instar);
		$subject = str_replace("'", "&#039;", $subject);
		$content = str_replace("'", "&#039;", $content);
		$tag = str_replace("'", "&#039;", $tag);
		$blog = str_replace("'", "&#039;", $blog);
		$instar = str_replace("'", "&#039;", $instar);
		
		$sql = "update $table set subject = '$subject', content = '$content', tag = '$tag', blog = '$blog', instar = '$instar' where num=$num";
		mysql_query($sql, $connect);  // $sql 에 저장된 명령 실행
	}
	else
	{
		...
		$sql = "insert into $table (id, name, nick, subject, content, tag, blog, instar, regist_day, ";
		$sql .= "file_name, file_copied) ";
		$sql .= "values('$userid', '$username', '$usernick', '$subject', '$content', '$tag', '$blog', '$instar', '$regist_day', ";
