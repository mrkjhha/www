...
<body>
    <?
        ...
        else     //검색 레코드가 있으면
        {
            ...
            //$row[id] ,.... $row[level]
            $sql ="select * from member where id='$id' and name='$name' and email='$mail'";
            ...
            $sql = "update member set pass=password('$ranpass') where id='$id' and name='$name' and email='$mail'";
            $result = mysql_query($sql, $connect);
        }
    }
    ?>
	<div id="wrap">
		...
		<article id="content">
			...
			<form  name="find_pw_form" method="post" action="./login.php">               
                <div id="name_hp_input">
					<dl>
						...
						<dd>
                            <strong>고객님의 임시비밀번호는 '<?=$ranpass?>' 입니다.</strong>
						</dd>
                        <dd>
                            아이디 : <?=$userid?> <br>
                            이름 : <?=$username?> <br>
