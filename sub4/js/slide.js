...
function slide_do(){
    for(var j=0; j<img_cnt; j++){                
        ...
        if(img_p[j]==p_center){
            $('.video_box li:eq('+j+')').css('zIndex','10').removeClass('other').removeClass('side').addClass('center');
        } else if(img_p[j]==p_center+per_set || img_p[j]==p_center-per_set){
            $('.video_box li:eq('+j+')').css('opacity','1').css('zIndex','5').removeClass('center').removeClass('other').addClass('side');
        } else if(img_p[j]==p_center+parseInt(img_cnt/2)*per_set){
            $('.video_box li:eq('+j+')').css('opacity','0').css('zIndex','0').removeClass('side').removeClass('center').addClass('other');
